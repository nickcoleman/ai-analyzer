# Stock-Analyst.md v8.0.6

**Version:** v8.0.6 - Freshness Unification  
...
### Step 5C: Apply Sector Constraint

**Purpose:** Enforce sector hard cap constraint (35% max sector allocation).

**Note on Hard Cap:** Hard cap = maximum size for NEW position being added. Does NOT include existing positions already in the same sector (already counted in current allocation).
...
sector_hard_cap = available_headroom

IF cash_adjusted_size <= sector_hard_cap:
  sector_constrained_size = cash_adjusted_size
  constraint_status = "UNCONSTRAINED"
  
ELIF cash_adjusted_size > sector_hard_cap:
  sector_constrained_size = sector_hard_cap
...
```
Current State:
  Metals sector: 30.7% of portfolio
  Sector cap: 35%
  Available headroom: 4.3%

New Position Analysis:
  Base conviction size: 5.0%
  After risk multiplier: 5.0% (no volatility adjustment in this example)
  After account cash multiplier: 5.0%
  
Hard Cap Application:
  Sector hard cap: 4.3%
  Final position size: MIN(5.0%, 4.3%) = 4.3%
  Status: CONSTRAINED by sector headroom

Post-Trade:
  Metals sector allocation: 30.7% + 4.3% = 35.0% (at cap)
```

**Handling Multiple New Positions Same Day:**
```
If SA approves 2+ new positions in same sector same day:
  Stock 1: Hard cap 4.3%
  Stock 2 (if approved same day): Hard cap reduced to (4.3% - Stock 1 size)
  
Example:
  Available headroom: 4.3%
  Stock 1 approved: 3.0% position
  Remaining headroom: 1.3%
  Stock 2 hard cap: 1.3% (max)
  
  QA and PO validate aggregate (Patch 3 aggregate validation)
  PO re-validates at execution (re-check if market moved)
```

**Calculation:**
```
sector_hard_cap_size = MAX(0, available_headroom)
final_sector_adjusted = MIN(cash_adjusted_size, sector_hard_cap_size)
```
...
**Stop-Loss Placement:**
```
Calculate based on conviction and conviction_size:
...
```
Based on thesis upside multiple:
...
End of Stock-Analyst.md v8.0.6